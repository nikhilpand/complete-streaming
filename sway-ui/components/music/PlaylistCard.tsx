...
export function PlaylistCard({ id, title, subtitle, artwork_url, className }: Props) {
  return (
    <Link href={`/playlist/${id}`} className={cn('group block cursor-pointer', className)}>
      ...
      {subtitle ? (
        ...
        <p className="text-xs text-[--muted] mt-0.5">Playlist</p>
      )}
    </Link>
  );
}

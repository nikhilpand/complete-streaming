...
from .models import FeedType, RecommendationContext, RecommendationExplanation, RecommendationItem, RecommendationResponse
...
class RecommendationEngine:
    ...
    def ingest_event(self,event):
        ...
        return ok
    def rebuild_profile(self,user_id):self.store.save_profile(self.profile_builder.build(user_id,self.store.user_events(user_id),self.store.tracks))
    ...
    def recommend(self,user_id,feed,context=None,limit=20,cursor=None):
        ...
        for idx,r in enumerate(page,1):
            rid=f"rec_{uuid.uuid4().hex[:16]}"; items.append(RecommendationItem(recommendation_id=rid,track=r.candidate.track,rank=idx,score=round(r.score,6),source=r.candidate.source,explanation=self._explanation(r.candidate))); new_seen.append(r.candidate.track.id)
